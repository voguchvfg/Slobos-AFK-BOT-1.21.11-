...
function createBot() {
  ...
  try {
    ...
    const currentUsername = botState.botNameIndex === 0 ? baseUsername : `${baseUsername}-${botState.botNameIndex}`;
...
function initializeModules(bot, mcData, defaultMove) {
  ...
  if (config.utils['auto-auth'].enabled) {
    ...
    setTimeout(() => {
      bot.chat(`/trigger password set ${password}`);
      console.log('[Auth] Sent trigger login command');
    }, 1000);
  }
